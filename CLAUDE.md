...
- `lookup_order` / `get_product_details` / `lookup_product` — direct SQLite via `MarketSphereBackend` (`src/backend.py`, wraps `db/marketsphere.db`, tables `orders`/`products`).
